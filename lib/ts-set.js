...
(async () => {
  ...
  try {
    ...
    if (args[0]) {
      ...
      const givenName = await question(chalk.green('📝  what will be the name of your new project? ➤ '));
    ...
    await fs.writeJSON(`${projectName}/package.json`, packageJson(projectName), {
      spaces: 2,
      EOL: os.EOL,
    });
    ...
    await exec(`cd ${projectName} && npm i --save-dev ${devDependencies.join(' ')}`, {
      stdio: 'inherit',
    });
